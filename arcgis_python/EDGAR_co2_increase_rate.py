...
for raster in rasters:
    ...
    if raster == first_year:
        ...
        temp_saveDiff_1990 = 'edgar_diff_1990_' + raster[-8:-4]
        try:
    ...
    if raster == last_year:
        ...
        temp_minus = arcpy.env.workspace + '\\edgar_' + temp_mod + '_log'
        temp_saveDiff_1990 = 'edgar_diff_1990_' + raster[-8:-4]
        temp_saveIncrease = 'edgar_increase_' + temp_mod + '_' + raster[-8:-4]
        temp_saveIncrease_points = 'edgar_increase_points' + temp_mod + '_' + raster[-8:-4]
        try:
            # calculate raster
            raster = arcpy.env.workspace + '\\' + raster
            raster_diff = Raster(raster) - Raster(raster_1990)
    ...
    temp_year_increase = arcpy.env.workspace + '\\edgar_' + temp_year_minus + '_log'

    # 设置保存路径
    saveDiff_1990 = 'edgar_diff_1990_' + temp_year
    saveIncrease = 'edgar_increase_' + temp_year_minus + '_' + temp_year
    saveIncrease_points = 'edgar_increase_points' + temp_year_minus + '_' + temp_year
