...
def TCalculator(tt, rcp, yrs, yre):
    ...
    while date_counter < TDate['end']:
        ...
        save_raster = arcpy.env.workspace + os.sep + tt + '_anomaly_' + rcp + '_' + date_counter.isoformat().replace('-', '')[:-2]
        avg_days = len(temp_raster)
        avg_stander = arcpy.env.workspace + os.sep + ('tas_historical_avg_%02d' % date_counter.month)
        ...
        avg_raster = (avg_raster / avg_days) - Raster(avg_stander)
...
def THistoricalAvg():
    ...
    rasters = SelectInYear(arcpy.ListRasters('tas_historical*'), TDate)
    ...
    while month_countor < 13:
        ...
        for tr in temp_raster:
            try:
                ...
                print arcpy.GetMessages()
                print 'Calculate historical average failed: %s at %s' % (tr, date_counter.isoformat())

        avg_raster = avg_raster / avg_months
        avg_raster.save(save_raster)
        print 'Calculate historical average successed: %s' % date_counter.isoformat().replace('-', '')[:-2]
        month_countor += 1
...
def SelectInYear(rasterslist, tdate):
    ...
    for rl in temp_rasterlist:
        ...
        if int(rl[-8:-4]) >= tdate['start'].year:
            if int(rl[-8:-4]) <= tdate['end'].year:
                temp_re.append(rl)
...
# ============================================================================
# 设置arcpy临时工作空间
arcpy.env.workspace = 'E:\\workplace\\CarbonProject\\geodatabase\\temperature.gdb'
...
# 列出目录下所有.nc文件
nc_path = "E:\\workplace\\CarbonProject\\DATA\\rcp2p6\\tasmin"
...
# calculate avg
# historical 
TCalculator('tasmin', 'rcp4p5', 2006, 2099)
# print process report
ffl_avg()
